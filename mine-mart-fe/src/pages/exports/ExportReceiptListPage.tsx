...
import { useExportReceipts } from '@/hooks/useExport';
import { useUsers } from '@/hooks/useUser';
import { useNavigate } from 'react-router-dom';
import dayjs from 'dayjs';

const ExportReceiptListPage = () => {
  const { data: receipts, isLoading } = useExportReceipts();
  ...
  const filteredReceipts = useMemo(() => {
    ...
    // Sort theo exportDate (mới nhất trước)
    if (result) {
      result = [...result].sort((a: { exportDate: string }, b: { exportDate: string }) => {
        const dateA = dayjs(a.exportDate).valueOf();
        const dateB = dayjs(b.exportDate).valueOf();
    ...
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <Space>
          ...
          <Button type="primary" icon={<PlusOutlined />} onClick={() => navigate('/exports/create')}>
            Tạo phiếu xuất
          </Button>
        </Space>
      </div>
...
export default ExportReceiptListPage;
